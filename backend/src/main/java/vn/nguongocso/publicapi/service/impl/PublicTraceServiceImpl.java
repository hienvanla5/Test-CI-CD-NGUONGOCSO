...
public class PublicTraceServiceImpl implements PublicTraceService {
	...
	public PublicTraceResponse getPublicTrace(String codeValue, Double latitude, Double longitude, String location,
		...
		if (isRecalled) {
			recallMessage = recallRepository.findTopByShipmentOrderByRecalledAtDesc(shipment).map(Recall::getReason)
		...
		// Ghi nhận lượt quét
		TraceCodeScanLog scanLog = TraceCodeScanLog.builder().traceCode(traceCode).scannedAt(LocalDateTime.now())
				.ipAddress(ipAddress).userAgent(userAgent)
				.latitude(latitude != null ? BigDecimal.valueOf(latitude) : null)
				.longitude(longitude != null ? BigDecimal.valueOf(longitude) : null)
				.location(location != null && !location.isBlank() ? location : "Không xác định").isAbnormal(false)
				.build();
		...
		if (shipment.getProductionLot() != null) {
			...
			List<ChainEvent> allUnassignedEvents = chainEventRepository
					.findByShipmentIsNullAndEventTypeIn(List.of(ChainEventType.HARVEST, ChainEventType.PACKAGING));

			productionLotEvents = allUnassignedEvents.stream().filter(event -> {
				Map<String, Object> data = parseEventData(event.getEventData());
				Object lotId = data.get("productionLotId");
				return lotId != null && productionLotId.toString().equals(lotId.toString());
			}).toList();
		}
		...
		List<PublicChainEventItem> publicEvents = allEvents.stream().map(this::convertToPublicEvent).toList();
	...
	private PublicChainEventItem convertToPublicEvent(ChainEvent event) {
		...
		return PublicChainEventItem.builder().eventType(event.getEventType().name()).eventData(filteredData)
				.recordedAt(event.getRecordedAt()).latitude(latitude).longitude(longitude).build();
	}
	...
	private PublicCertificationResponse toPublicCertification(ProductionLotCertification lotCertification) {
		...
		return PublicCertificationResponse.builder().certificationId(certification.getId())
				.certificationName(certification.getName()).certificationCode(certification.getCode())
				.issuedBy(certification.getIssuedBy()).issueDate(certification.getIssueDate())
				.expiryDate(certification.getExpiryDate()).status(status).statusLabel(status.getLabel()).build();
	}
}