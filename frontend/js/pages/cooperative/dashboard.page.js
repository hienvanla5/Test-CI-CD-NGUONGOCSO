...
/*
 * Màn quản lý thành viên của hợp tác xã
...
// Set org name
if (orgNameDisplay) {
    orgNameDisplay.textContent = user.organizationName || "—";
}
...
function filterMembers(query) {
    if (!query) {
        ...
        updateSummary(membersCache);
        return;
    }

    var filtered = membersCache.filter(function (member) {
        var name = (member.fullName || "").toLowerCase();
...
async function loadMembers() {
    ...
    try {
        ...
        var allMembers = response.data || [];

        // Filter out system administrators (VT-01)
        var visibleMembers = allMembers.filter(function (member) {
...
function renderMembers(members) {
    ...
    members.forEach(function (member) {
        ...
        joinedCell.setAttribute("data-label", "Joined Date");

        if (member.joinedAt) {
            try {
                var date = new Date(member.joinedAt);
                joinedCell.textContent = date.toLocaleDateString("en-US", {
                    year: "numeric",
                    month: "short",
                    day: "numeric"
                });
            } catch (e) {
                joinedCell.textContent = member.joinedAt;
            }
        } else {
...
async function loadRoles() {
    try {
        ...
        if (Array.isArray(response)) {
            // Roles are returned directly as an array
            rolesCache = response;
...
function populateRoleSelects() {
    // Filter out VT-01 - not assignable by cooperative managers
...
assignRoleSave.addEventListener("click", async function () {
    ...
    try {
        ...
        // Reload members list
        await loadMembers();
        // Reload roles to refresh dropdowns
        await loadRoles();
        ...
        var message = error.message || "Failed to assign role.";

        // Handle 403 from backend
        if (message.indexOf("403") !== -1 || message.toLowerCase().indexOf("không thể gán") !== -1) {
            message = "You do not have permission to assign this role.";
        }

        assignRoleMessage.textContent = message;
...
function openAddMemberModal() {
    ...
    // Re-populate role dropdown in case it changed
    populateRoleSelects();
...
addMemberForm.addEventListener("submit", async function (event) {
    ...
    try {
        ...
        if (message.toLowerCase().indexOf("username") !== -1 && message.toLowerCase().indexOf("tồn tại") !== -1) {
        ...
        if (message.toLowerCase().indexOf("403") !== -1 || message.toLowerCase().indexOf("không thể tạo") !== -1) {
            message = "You do not have permission to create a member with this role.";
        }
