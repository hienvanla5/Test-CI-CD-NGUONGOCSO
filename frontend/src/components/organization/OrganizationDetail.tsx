...
import { getOrganizationDetail, createOrganizationMember } from "@/api/organizationApi";
        ...
        <div className="space-y-6">
            <Card>
                ...
                <CardContent className="grid grid-cols-2 gap-4">
                    <div><b>Mã:</b> {data.profile.code}</div>
                    <div><b>Tên:</b> {data.profile.name}</div>
                    <div><b>Loại:</b> {ORGANIZATION_TYPE_LABELS[data.profile.type] ?? data.profile.type}</div>
                    <div><b>Email:</b> {data.profile.email}</div>
                    <div><b>SĐT:</b> {data.profile.phone}</div>
                    <div><b>Địa chỉ:</b> {data.profile.address}</div>
                    <div>
                        <b>Trạng thái:</b>
                        ...
                        <TableBody>
                            {data.members.map((m) => (
                                <TableRow key={m.id}>
                                    ...
                                    <TableCell>{m.roleCode ? getRoleLabel(m.roleCode) : m.roleName}</TableCell>
