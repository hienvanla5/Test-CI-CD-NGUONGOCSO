...
import type { AreaAnalysisStats, AreaSeasonStats } from '@/types/cropAreaAnalysis';
        ...
        <Table>
          ...
          <TableBody>
            {data.map((area) => {
              ...
              return (
                <>
                  <TableRow key={area.farmAreaId} className="cursor-pointer hover:bg-muted/50" onClick={() => toggleExpand(area.farmAreaId)}>
                    <TableCell className="font-medium">{area.farmAreaName}</TableCell>
                    <TableCell className="text-right">{area.areaSize}</TableCell>
                    <TableCell className="text-right">{area.totalLots}</TableCell>
                    ...
                    <TableCell className="text-right">{area.organizationName}</TableCell>
                  ...
                  {isExpanded && area.seasons && area.seasons.length > 0 && (
                    <TableRow>
                            ...
                            <TableBody>
                              {area.seasons.map((season) => (
                                <TableRow key={season.seasonCode}>
                                  ...
                                  <TableCell className="text-right">{season.lotCount}</TableCell>
