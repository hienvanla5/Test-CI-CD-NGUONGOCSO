...
- Env is loaded from `backend/.env` via `spring-dotenv` (DB_HOST/DB_PORT/DB_NAME/DB_USERNAME/DB_PASSWORD, defaults in `application.properties`). `backend/.env` is tracked but also gitignored — do not add secrets there.
